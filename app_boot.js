...
async function syncPull(){
  ...
  try{
    ...
    if(R.lastPos&&(!S.lastPos||(R.lastPos.d||'')>(S.lastPos.d||''))){
      S.lastPos=R.lastPos; await kvSet('lastpos',S.lastPos);
      if(S.lastPos.lvl===2||S.lastPos.lvl===3)UI.lvl=S.lastPos.lvl;
      ch=true;}
...
/* build: v1.4.8 2026-07-20 — 🎲 дистракторы v2: вместо детерминированного top-3 берётся пул top-14 и n=3 сэмплируются взвешенно по позиции (sampleK), гарантии «≥1 из темы» и «≥1 изученное» тоже выбирают случайного кандидата (pickRnd); штраф за длину смягчён (шаги по 3 симв., потолок 4), добавлен бонус за изученность (−4) → 2–3 из 3 дистракторов знакомы. Пул растёт сам: seen пишется в S.srs в finishSession, поэтому слова только что пройденного урока участвуют со следующего. То же для pdistract (фразы). 🧹 Банк: 929 → 892 слова — удалены 36 буквальных дублей ky=ru (заимствования: депутат, система, университет…) и «манас»; дочищены 13 подсказок, проскочивших чистку v1.4.7 (алып, таап, жаап, эли, ордун, күнкү, бу, абадан, туш, абалы, ала, чет, жайында). */
/* build: v1.4.7 2026-07-14 — 🧹 чистка переводов wordbank (63 поля): убраны «(от «X»)» и уточнения-подсказки, содержащие само слово или его корень в «» (правила R1/R2/R3 + 6 ручных глоссов для «форма «X»»; тилдүү = «-язычный»; дистракторы проверены симуляцией на реальном коде — 1858 наборов, 0 нарушений; бэкап wordbank в .claude/archive/wordbank_backup_20260714.json, words.json синхронизирован). Во фразах/предложениях тап по слову → попап с переводом из банка (kyTap/kyLookup в app_core: точное совпадение или основа-префикс ≥3 букв, пунктирное подчёркивание только у найденных, 🔊 в попапе, «≈ по основе слова» при неточном; попап закрывается кликом мимо и при render). */
/* build: v1.4.6 2026-07-12 — «Продолжить» ведёт в раздел последнего пройденного урока (lastPos={lvl,uid,d} в kv+sync: вкладка Слова/Фразы/Предложения восстанавливается при старте, цель «Продолжить» — недоделанные уроки от последнего юнита вперёд, иначе firstUndone). В «Мои слова» кнопка «🔁 Повторить»: случайные слова/фразы из текущего фильтра (учу/знаю/сложные/все + поиск), свободная тренировка через practiceScreen → finishSession(practice=true) НЕ двигает SRS (due/step/seen); XP/стрик/квесты начисляются. */
